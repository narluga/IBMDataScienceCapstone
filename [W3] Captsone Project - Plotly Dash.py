...
site_df = spacex_df.groupby('Launch Site').first().reset_index()
#print(site_df)

dropList = [{'label': 'All Sites', 'value':'ALL'}]

# Crate list of label/value dictionary.
for index, row in site_df.iterrows():
    rowDic = {'label': row['Launch Site'], 'value': row['Launch Site']}
    dropList.append(rowDic)
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df = filtered_df.astype({'class': str})
        print(filtered_df)
...
launch success rate? B5, v1.0 is lowest